...
const fetchInchLPTokens = async () => {
  return fetch(`https://governance.1inch.io/v1.1/protocol/pairs`).then((res) =>
    res.json()
  );
};

export const queryLPTokenBalances = async (userAddress) => {
  try {
    const LPtokens = await fetchInchLPTokens();
...
const processResult = (LPtokens, tokenBalances) => {
  ...
  tokenBalances.map((tokenBalance) => {
    ...
    const id1 = Object.keys(tokens).find(
      (id) => tokens[id].address === token0.address
    );
    const id2 = Object.keys(tokens).find(
      (id) => tokens[id].address === token1.address
    );
    ...
    token0.image = `https://tokens.1inch.io/${token0.address}.png`;
    token1.image = `https://tokens.1inch.io/${token1.address}.png`;
    if (amount)
      output.push({
        id: pair,
        lpToken1: {
